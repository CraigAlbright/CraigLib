...
namespace CraigLib.Sound
{
    ...
    /// </summary>
    internal class LibsndfileMarshal : ILibsndfileMarshal
    {
        private readonly ILibsndfileArrayMarshal _mArrayMarshaller;

        /// <summary>
        /// Initializes a new <see cref="LibsndfileMarshal"/> with the 
        /// default <see cref="ILibsndfileArrayMarshal"/> implementation.
        /// </summary>
        internal LibsndfileMarshal()
            : this(new LibsndfileArrayMarshal())
        {
        }

        /// <summary>
        /// Initializes a new <see cref="LibsndfileMarshal"/> with 
        /// the <paramref name="arrayMarshaller"/> implementation.
        /// </summary>
        /// <param name="arrayMarshaller"><see cref="ILibsndfileMarshal"/> implementation to use.</param>
        /// <remarks>
        /// This constructor should only be used for mocking.
        /// </remarks>
        internal LibsndfileMarshal(ILibsndfileArrayMarshal arrayMarshaller)
        {
            if (arrayMarshaller == null)
                throw new ArgumentNullException("arrayMarshaller");

            _mArrayMarshaller = arrayMarshaller;
        }
        ...
        public T[] MemoryHandleToArray<T>(UnmanagedMemoryHandle memory)
            where T : struct
        {
            return _mArrayMarshaller.ToArray<T>(memory);
        }
    }
}
